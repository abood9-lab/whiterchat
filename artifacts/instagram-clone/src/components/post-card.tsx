...
export function PostCard({ post }: { post: Post }) {
  ...
  const locallyLiked = isPostLocallyLiked(post.id);
  const locallySaved = isPostLocallySaved(post.id);

  const [userReaction, setUserReaction] = useState<string | null>(
    (post as any).userReaction ?? (post.isLiked || locallyLiked ? "❤️" : null)
  );
  const [likesCount, setLikesCount] = useState<number>(
    (post.likesCount || 0) + (locallyLiked && !post.isLiked ? 1 : 0)
  );
  const [reactionsSummary, setReactionsSummary] = useState<
    { emoji: string; count: number }[]
  >((post as any).reactionsSummary ?? []);

  const [isSaved, setIsSaved] = useState(post.isSaved || locallySaved);

  useEffect(() => {
    const isLocalL = isPostLocallyLiked(post.id);
    const isLocalS = isPostLocallySaved(post.id);

    setUserReaction((post as any).userReaction ?? (post.isLiked || isLocalL ? "❤️" : null));
    setLikesCount((post.likesCount || 0) + (isLocalL && !post.isLiked ? 1 : 0));
    setIsSaved(post.isSaved || isLocalS);
  }, [post]);
  ...
  const { data: suggestedData } = useGetSuggestedUsers({
    query: { enabled: !!user && shareOpen, retry: false } as any,
  });

  const followingList: UserSummary[] = useMemo(() => {
    const list: UserSummary[] = [];
    if (Array.isArray(followingData)) {
      list.push(...(followingData as UserSummary[]).filter(Boolean));
    } else if (followingData && Array.isArray((followingData as any).users)) {
      list.push(...((followingData as any).users as UserSummary[]).filter(Boolean));
    }

    if (Array.isArray(suggestedData)) {
      list.push(...(suggestedData as UserSummary[]).filter(Boolean));
    } else if (suggestedData && Array.isArray((suggestedData as any).users)) {
      list.push(...((suggestedData as any).users as UserSummary[]).filter(Boolean));
    }

    // Deduplicate by ID
    const seen = new Set<string>();
    return list.filter((u) => {
      if (!u || !u.id || seen.has(u.id) || u.id === user?.id) return false;
      seen.add(u.id);
      return true;
    });
  }, [followingData, suggestedData, user?.id]);
  ...
  const handleReact = async (emoji: string) => {
    ...
    setPostLocallyLiked(post.id, !!newReaction);
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPostLocallyLiked(post.id, !!data.userReaction);
      }
    } catch {
      // Keep optimistic state in local storage so heart stays red
    }
  };
  ...
  const handleSave = async () => {
    ...
    setPostLocallySaved(post.id, nextSaved);
    try {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Post by @${post.author?.username || "user"} on WhiterChat`,
          text: post.caption || "Check out this post on WhiterChat!",
          url: postUrl,
        });
      } catch {
  ...
  const handleShare = () => {
    if (navigator.share && typeof window !== "undefined" && window.innerWidth < 640) {
      handleNativeShare();
    } else {
      setShareOpen(true);
    }
        ...
        <SheetContent side="bottom" className="rounded-t-2xl max-h-[80vh] flex flex-col">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            <button
              ...
              className="flex flex-col items-center gap-1.5 py-3 rounded-xl border border-border bg-muted/30 hover:bg-muted transition-colors"
            >
            ...
            <button
              ...
              className="flex flex-col items-center gap-1.5 py-3 rounded-xl border border-border bg-muted/30 hover:bg-muted transition-colors disabled:opacity-50"
            >
