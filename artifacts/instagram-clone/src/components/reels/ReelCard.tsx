...
function getAuthToken(): string {
  ...
  return localStorage.getItem("pixlr_token") || localStorage.getItem("whiterchat_token") || "";
}
...
export function ReelCard({
  ...
  // Sync props with local persistence
  useEffect(() => {
    const locallyLiked = isReelLocallyLiked(reel.id);
    const locallySaved = isReelLocallySaved(reel.id);

    const activeLiked = reel.isLiked || locallyLiked;
    const activeSaved = reel.isSaved || locallySaved;

    setIsLiked(activeLiked);
    setLikesCount(reel.likesCount + (locallyLiked && !reel.isLiked ? 1 : 0));
    setMyReaction(reel.myReaction || (activeLiked ? "❤️" : null));
    setIsSaved(activeSaved);
  ...
  const handleLike = async (customEmoji: string = "❤️") => {
    ...
    const willBeLiked = !(wasLiked && myReaction === customEmoji);
    setReelLocallyLiked(reel.id, willBeLiked);
    ...
    try {
      ...
      setReelLocallyLiked(reel.id, data.liked);
    } catch {
      // Keep optimistic state in local storage so UI doesn't flicker
    }
  };
  ...
  const handleSave = async () => {
    ...
    setReelLocallySaved(reel.id, nextState);
    ...
    try {
      ...
      setReelLocallySaved(reel.id, data.isSaved);
      toast({
