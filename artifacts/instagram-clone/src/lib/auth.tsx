...
let _latestToken: string | null =
  (typeof window !== "undefined"
    ? localStorage.getItem(TOKEN_KEY) || localStorage.getItem("whiterchat_token")
    : null);

setAuthTokenGetter(() => {
  if (_latestToken) return _latestToken;
  if (typeof window !== "undefined") {
    return localStorage.getItem(TOKEN_KEY) || localStorage.getItem("whiterchat_token");
  }
  return null;
});
...
async function attemptRefresh(): Promise<string | null> {
  ...
  try {
    ...
    localStorage.setItem("whiterchat_token", data.token);
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = (newToken: string, newRefreshToken: string, newUser: UserProfile) => {
    ...
    localStorage.setItem("whiterchat_token", newToken);
  ...
  const switchAccount = async (accountId: string): Promise<boolean> => {
    ...
    localStorage.setItem("whiterchat_token", target.token);
  ...
  const logout = () => {
    ...
    localStorage.removeItem(TOKEN_KEY);
    localStorage.removeItem("whiterchat_token");
    localStorage.removeItem(REFRESH_KEY);
    _latestToken = null;
    setToken(null);
    queryClient.clear();
