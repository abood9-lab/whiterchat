...
export default function Create() {
  ...
  const StepHeader = () => {
    const stepNumbers: Record<Step, string> = { select: "1/3", edit: "2/3", details: "3/3" };
    return (
      <>
        {/* Mobile Compact Indicator */}
        <span className="sm:hidden text-xs font-bold px-2 py-0.5 rounded-full bg-primary/10 text-primary">
  ...
  return (
    <div className="max-w-5xl mx-auto pt-2 sm:pt-4 pb-24 md:pb-12 px-3 sm:px-6 space-y-6">
      ...
      {step === "edit" && (
        <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
          {/* Top Bar */}
          <div className="flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 border-b border-border bg-muted/20 gap-2 flex-wrap sm:flex-nowrap">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setStep("select")}
              className="gap-1 text-xs font-semibold px-2 sm:px-3 h-8 sm:h-9"
            >
              <ChevronLeft className="w-4 h-4" /> <span className="hidden xs:inline">Back to</span> Upload
            </Button>
            <StepHeader />
            <Button
              onClick={() => setStep("details")}
              size="sm"
              className="rounded-xl px-3 sm:px-5 text-xs font-semibold gap-1 sm:gap-1.5 shadow-sm h-8 sm:h-9"
            >
              Next<span className="hidden sm:inline">: Details</span> <ChevronRight className="w-4 h-4" />
            </Button>
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 border-b border-border bg-muted/20 gap-2 flex-wrap sm:flex-nowrap">
            <Button
              ...
              className="gap-1 text-xs font-semibold px-2 sm:px-3 h-8 sm:h-9"
            >
              <ChevronLeft className="w-4 h-4" /> <span className="hidden xs:inline">Back to</span> Edit
            </Button>
            <StepHeader />
            <Button
              onClick={handleSubmit}
              disabled={isPosting || items.length === 0}
              className="rounded-xl px-3 sm:px-6 text-xs font-bold shadow-md shadow-primary/20 gap-1.5 h-8 sm:h-9"
            >
              {isPosting ? (
                <>
                ...
                <>
                  ...
                  <span>Share<span className="hidden sm:inline"> Post</span></span>
                </>
              )}
            </Button>
          </div>
