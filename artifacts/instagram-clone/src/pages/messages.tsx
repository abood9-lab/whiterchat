...
import { ChatLongPressSheet, type ChatTarget } from "@/components/chat/ChatLongPressSheet";
...
async function apiRequest(path: string, opts: RequestInit = {}) {
  ...
  const r = await fetch(apiUrl(`/api/${cleanPath}`), {
    ...opts,
...
export default function Messages() {
  ...
  // Chat Long Press Context Sheet state
  const [longPressTarget, setLongPressTarget] = useState<ChatTarget | null>(null);
  const convTouchTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const handleConvTouchStart = (conv: any) => {
    convTouchTimerRef.current = setTimeout(() => {
      if (typeof window !== "undefined" && window.navigator && window.navigator.vibrate) {
        window.navigator.vibrate(50);
      }
      const name = conv.isGroup
        ? (conv.groupName || "Group")
        : (conv.otherUser?.fullName || conv.otherUser?.username || "User");
      setLongPressTarget({
        id: conv.id,
        name,
        username: conv.otherUser?.username,
        avatarUrl: conv.isGroup ? conv.groupAvatarUrl : conv.otherUser?.avatarUrl,
  ...
  const handleConvTouchEnd = () => {
    if (convTouchTimerRef.current) {
      clearTimeout(convTouchTimerRef.current);
      convTouchTimerRef.current = null;
    }
  };
  ...
  return (
        ...
        <ScrollArea className="flex-1">
          ...
          {(conversations as any[])?.map((conv: any) => (
            <div
              key={conv.id}
              className={cn(
                "flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-secondary/50 transition-colors group relative select-none",
                activeConvId === conv.id && "bg-secondary",
                conv.isArchived && "opacity-60"
              )}
              onClick={() => handleSelectConv(conv.id)}
              onTouchStart={() => handleConvTouchStart(conv)}
    ...
    {/* Chat Item Long Press Bottom Sheet */}
    <ChatLongPressSheet
      target={longPressTarget}
      open={!!longPressTarget}
      onClose={() => setLongPressTarget(null)}
      onMuteToggle={(t) => handleMute(t.id, t.isMuted ?? false)}
      onBlock={(t) => {
        if (t.id) {
          apiRequest(`conversations/${t.id}/block`, { method: "POST" }).then(() => {
            queryClient.invalidateQueries({ queryKey: getGetConversationsQueryKey() });
          }).catch(() => {});
        }
      }}
      onHideSecretVault={(t) => {
        setVaultAddConvId(t.id);
        setVaultAddConvUser(t.username ?? t.name);
        setShowVault(true);
      }}
      onArchiveToggle={(t) => handleArchive(t.id, t.isArchived ?? false)}
    />
    </>
  );
}
