...
export default function ReelsPage() {
  ...
  const getLocalComments = (reelId: string): CommentData[] => {
    try {
      const raw = localStorage.getItem(`whiterchat_reel_comments_${reelId}`);
      if (!raw) return [];
  ...
  const saveLocalComments = (reelId: string, list: CommentData[]) => {
    try {
      localStorage.setItem(`whiterchat_reel_comments_${reelId}`, JSON.stringify(list));
    } catch {}
  };
  ...
  const handleOpenComments = async (reel: ReelData) => {
    ...
    const local = getLocalComments(reel.id);
    setComments(local);
    ...
    try {
      ...
      if (res.ok) {
        ...
        for (const lc of local) {
          if (!combined.some((sc) => sc.id === lc.id || sc.text === lc.text)) {
            combined.push(lc);
          }
        }
        setComments(combined);
        saveLocalComments(reel.id, combined);
      }
    } catch {}
  ...
  const handleCommentSubmit = async (data: CommentSubmitData) => {
    ...
    // Get active user details for optimistic comment
    let currentUser = { id: "me", username: "you", avatarUrl: null, fullName: "You" };
    try {
      const storedUser = localStorage.getItem("whiterchat_user") || localStorage.getItem("pixlr_user");
      if (storedUser) currentUser = JSON.parse(storedUser);
    } catch {}

    const tempCommentId = "opt-" + Date.now();
    ...
    saveLocalComments(activeCommentsReel.id, updatedComments);
    ...
    try {
      const token = getAuthToken();
      let res = await fetch(apiUrl(`/api/reels/${activeCommentsReel.id}/comments`), {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        res = await fetch(apiUrl(`/api/posts/${activeCommentsReel.id}/comments`), {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...(token ? { Authorization: `Bearer ${token}` } : {}),
      ...
      if (res.ok) {
        ...
        setComments((prev) => {
          const replaced = prev.map((c) => (c.id === tempCommentId ? serverComment : c));
          saveLocalComments(activeCommentsReel.id, replaced);
          return replaced;
        });
      }
    } catch {
      // Local storage comment cache already preserved the comment
    }
  };
